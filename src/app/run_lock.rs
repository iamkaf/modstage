...
pub(super) fn fetch_locked_assets(
    ...
    for block in lock.split("[[asset]]").skip(1) {
        ...
        missing_assets.push(LockedAssetDownload {
            name,
            hash,
            url,
            sha256: block_string_value(block, "sha256"),
        });
    }

    fetch_locked_asset_objects(cache_dir, &missing_assets)?;

    for asset in missing_assets {
        let object_path = asset_object_dir(cache_dir, &asset.hash).join(&asset.hash);
        verify_file_sha1("locked asset", &asset.name, &object_path, &asset.hash)?;
    }
...
fn fetch_locked_asset_objects(
    cache_dir: &Path,
    assets: &[LockedAssetDownload],
) -> Result<(), String> {
    if assets.is_empty() {
        return Ok(());
    }

    let workers = asset_download_concurrency().min(assets.len());
        ...
        for _ in 0..workers {
            scope.spawn(|| {
                loop {
                    ...
                    if let Err(error) = fetch_locked_asset_object(cache_dir, asset) {
    ...
    if let Some(error) = first_error
        .into_inner()
        .map_err(|_| "asset download worker panicked while holding error lock".to_string())?
    {
        return Err(error);
    }
...
fn fetch_locked_asset_object(cache_dir: &Path, asset: &LockedAssetDownload) -> Result<(), String> {
    let object_path = fetch_to_cache(
        &asset.url,
        &asset_object_dir(cache_dir, &asset.hash),
        &asset.hash,
    )?;
    verify_file_sha1("locked asset", &asset.name, &object_path, &asset.hash)?;
    if let Some(expected) = &asset.sha256 {
        verify_file_hash("locked asset", &asset.name, &object_path, expected)?;
    }
    Ok(())
}

fn asset_download_concurrency() -> usize {
    std::env::var("MODSTAGE_DOWNLOAD_CONCURRENCY")
        .ok()
